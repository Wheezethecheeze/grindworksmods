...
func fill_maze() -> void:
	...
	for i in grid_size.x:
		for j in grid_size.y:
			...
			var new_unit: Node3D = base_unit.duplicate(true)
...
func generate_maze() -> void:
	...
	var cell: GridSpace = unvisited_cells[RNG.channel(RNG.ChannelMazeSizes).randi() % unvisited_cells.size()]
